...
但是对于大多数中国程序员，貌似将定义在类里面的类称为内部类不是更符合文意么？也许这就是为什么会有这么多人一直认为静态内部类的存在。嘛，反正不管怎么称呼，只要知道代表什么意思就好，细节什么的不要在意（光哥：所以不注意细节的你写的代码才这么一大堆bug么？还不赶紧去改（怒吼））。我这里采用Java语言规范上说的。
